...
En este caso vemos como el bucle toma una pausa entre cada iteración y luego hace el calculo.
...
2.3 Haga un bucle `apply` para calcular el error estandar para el ancho de sépalo en el juego de datos 'iris'.
...
&nbsp;
4.1 Convierta en un bucle `while` el bucle `repeat` del ejercicio 3.1


&nbsp;

4.2 Haga un bucle `while` que se detenga si la correlación es superior a 0.8 o si el bucle ha estado ejecutándose durante más de 10 segundos (consejo: use la función` difftime` y / o `as.numeric`)

&nbsp;

4.3  Haga un bucle `repeat` que se detenga solo si la correlación es mayor que 0.5 pero menor que 0.55
