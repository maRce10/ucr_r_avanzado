...
## Añadir más de un predictor: regresión múltiple
...
<font size = 6><center>Variables indicadoras (Dummy coding)</center></font>
...
#generar variables
y <- rpois(n = 3, lambda = 6.5) # lambda = tasa promedio de exitos 
...
Vamos a simular algunos datos que se asemejan a ese escenario. Tenemos dos predictores continuos (x1) y una respuesta continua (y). Cada muestra procede de 1 de 8 poblaciones diferentes (pops):
...
El modelo detectó correctamente el patrón simulado y la estimación de $\ta_1$ (`r fixef(mix_eff_mod)[2]`) es muy cercana al valor simulado.
