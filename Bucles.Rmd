...
Creemos un bucle que calcula las correlaciones entre variables aleatorias usando un bucle `for`:
...
1.1 Haga un bucle `for` que devuelva el número de filas para cada juego de datos de ejemplo

&nbsp;

1.2 Haga un bucle `for` que devuelva el número de filas y columnas para cada juego de datos de ejemplo

&nbsp;

1.3 El juego de datos de ejemplo "ChickWeight" describe el "peso versus la edad de los pollitos en diferentes dietas":
```{r, eval=F, echo=T}
...
knitr::include_graphics("./images/xapply.png")
...
2.1 Haga un bucle `lapply` equivalente al bucle `for` en el ejercicio 1.4 (*utilizando los datos 'ChickWeight' calcule la correlación entre peso y tiempo para cada Chick*)

&nbsp;

2.2  Haga un bucle `sapply` para calcular el mayor peso registrado para cada tipo de dieta (pista: `unique(ChickWeight$Diet)`, deberia devolver un valor por tipo de dieta). Nombre el vector resultante para que contenga el identificador de cada dieta.

&nbsp;

2.3 Haga un bucle `apply` para calcular la media de cada variable numérica en el juego de datos 'iris'.
...
&nbsp;
3.1 Haga un bucle `while` que se detenga solo si la correlación es mayor que 0.5 pero menor que 0.55

&nbsp;

3.2 Haga un bucle `while` que se detenga si la correlación es superior a 0.8 o si el bucle ha estado ejecutándose durante más de 10 segundos (consejo: use la función` difftime` y / o `as.numeric`)
...
4.1 Convierta en un bucle `repeat` el bucle `while` del ejercicio 1.2
...
E.1 Reúna los resultados del ejercicio 1.4 en un nuevo juego de datos con columnas para 'chick' y 'correlation' usando un bucle `for` (consejo: use `rbind`)
