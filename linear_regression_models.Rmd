...
b0 <- 1
b1 <- 1.3
subpop <- sample(0:8, size = n, replace = TRUE)
error <- rnorm(n = n, sd = 2)
...
# random variables
x1 <- rnorm(n = n)
y <- b0 + subpop * 2 + b1 * x1 + error

# create data frame
xy_data_subpop <- data.frame(x1, y, subpop = as.factor(subpop))


ggplot(data = xy_data_subpop, aes(x = x1, y = y, color = subpop)) + 
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) 

summary(lm(y ~ x1, data = xy_data_subpop))
...
mix_eff_mod <- lmer(formula = y ~ x1 + (1 | subpop))
