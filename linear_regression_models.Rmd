...
y[x1 < 0] <- rpois(n = sum(x1 < 0), lambda = 2)
y[x1 > 0] <- rpois(n = sum(x1 > 0), lambda = 3)

# create data frame
xy_data_pois <- data.frame(x1, y)
...
```{r}
lm_pois  <- lm(formula = y ~ x1, data = xy_data_pois)
...
- Try fitting a `lm()` model (so with a gaussian distribution), compare the results and check the residuals (`plot(model_name)`)
